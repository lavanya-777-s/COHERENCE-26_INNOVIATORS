            ...
            {TABS.map(tab => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 whitespace-nowrap ${isActive
                    ...
                    <div key={idx} className="bg-rose-50/50 p-4 rounded-xl border border-rose-100 hover:border-rose-300 transition-colors">
                      <div className="flex justify-between items-start gap-4">
                        ...
                        <div className="flex flex-col items-end gap-2">
                          <span className={`px-4 py-1.5 rounded-full text-sm font-bold text-white shadow-sm ${anomaly.risk === 'high' ? 'bg-rose-600' :
                ...
                <div className="flex bg-slate-100 p-1.5 rounded-xl border border-slate-200">
                  {['All', 'Success', 'Pending'].map((status) => (
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${filterStatus === status
                  ...
                  {filteredAllocations.slice().reverse().map((alloc) => (
                    <div key={alloc.id} className="bg-slate-50 p-5 rounded-2xl border border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-indigo-200 hover:shadow-md transition-all group">
                      <div className="flex items-start gap-4">
                        <div className={`p-3 rounded-xl ${alloc.status === 'Success' ? 'bg-emerald-100' : 'bg-amber-100'
                          }`}>
                      ...
                      <div className="flex flex-col items-end md:items-end justify-center">
                        <p className={`text-2xl font-extrabold tracking-tight ${alloc.status === 'Success' ? 'text-emerald-600' : 'text-amber-600'
                          }`}>+₹{alloc.amount}Cr</p>
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold mt-2 shadow-sm border ${alloc.status === 'Success'
                          ? 'bg-emerald-100 text-emerald-800 border-emerald-200'
                          : 'bg-amber-100 text-amber-800 border-amber-200'
                          }`}>
                          <span className={`w-1.5 h-1.5 rounded-full ${alloc.status === 'Success' ? 'bg-emerald-500' : 'bg-amber-500 animate-pulse'
                            }`}></span>
                          {alloc.status}
                        </span>
                      </div>
